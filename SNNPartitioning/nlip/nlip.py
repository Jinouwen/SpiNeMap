...
def optimizationFunction1(x, srcCrossbar, srcNeuron):    
    #finding the total number of spikes communicated between the crossbars.
    totalSpikeCount = 0
    spikeCount = 0
...
# Define the constraints one- at a time
...
for srcCrossbar in range(numGroups):
    for srcNeuron in range(numNeurons):
        if(optimizationFunction(x, srcCrossbar, srcNeuron) is not 0):
            m.Obj(optimizationFunction1(x, srcCrossbar, srcNeuron))
