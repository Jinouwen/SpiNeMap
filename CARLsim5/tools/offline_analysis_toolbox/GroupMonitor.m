    ...
    methods (Hidden, Access = private)
        ...
        function plotFrame(obj, frameNr, plotType, frameDur, dispFrameNr)
            ...
            elseif strcmpi(obj.plotType,'heatmap')
                ...
                frameR = obj.spkData(:,:,1,frameNr);
                frameG = obj.spkData(:,:,2,frameNr);
                frameB = obj.spkData(:,:,3,frameNr);
                size(obj.spkData)
                size(frameR)
                %imagesc(frame, [0 maxD])
                
                %a = image(frame,'CDataMapping','scaled')
                %imshow(frameR, [0 maxD]);
                imshow(frameG, [0 maxD]);
                %imshow(frameB, [0 maxD]);
                %hgexport(gcf,'/home/adarsha/NeuroXplore/toolchain/NeuroXplore/CARLsim5/images/ImageSmooth/outputms.jpg')
                %axis image equal
