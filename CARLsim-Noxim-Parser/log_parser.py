...
weight_table_file = open('../weight_table.txt', mode='w')
...
def spikingTimeParser(log_str):
    ...
    spikeCount = open('../spikeTraffic.txt', mode='w')
...
#pattern1 = re.compile(second_line_re + rest_line_re, flags=0) 
        
        #if(pattern)
    
    #spike_table = pattern1.finditer(log_str)
    ...
    #for i in time_table:
       # time_list = i.group().replace('|', '').strip()
        ...
        #if neuron_id not in spiking_time and count == 0:
            #when the need
            #spiking_time[neuron_id] = 0
        #else:
            #if neuron_id == 0:
                #count = count + 1
                #localCount = localCount + iterator[count]

           # neuron_id = neuron_id + localCount
            #spiking_time[neuron_id] = 0
   
    
    #for i in spike_table: 
        #time_list = i.group().replace('|', '').strip()
       # time_list = re.compile('\s+', flags=0).split(time_list)
        #print(time_list)
        #neuron_id = int(time_list.pop(0))
        #rate = float(time_list.pop(0))
        #time_list = [int(i) for i in time_list]
        #spiking_time[neuron_id] = time_list
    
    #for i in spiking_time: 
        #print(spiking_time[i])
    
   # return spiking_time


log_file = open('carl.log', mode='r')  #'/home/adarsha/Research/Noxim/noxim-extended-for-neuromorphic/bin/carl-logs/8x8.log', mode='r')
...
#neuronNumberParser(log_file_string)
'''
...
log_file1 = open('carl.log', mode='r')  #'/home/adarsha/Research/Noxim/noxim-extended-for-neuromorphic/bin/carl-logs/8x8.log', mode='r')
log_file_lines1 = log_file1.readlines()
spiking_time = spikingTimeParser(log_file_lines1)
