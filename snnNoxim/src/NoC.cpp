...
void NoC::buildMesh() {
    ...
    for (map<int, HubConfig>::iterator it =
        ...
        if (no_channels > 1) {
            cerr << " WARNING: Power model currently not supporting multi-channel "
                    "per hub, using default_tx_energy"
                 << endl;
        }
    ...
    if (GlobalParams::routing_algorithm == ROUTING_TABLE_BASED) {
        ...
        assert(grtable.load(GlobalParams::routing_filename.c_str()));
        std::cout << "Done" << endl;
    } else if (GlobalParams::routing_algorithm == ROUTING_FILE_BASED) {
        std::cout << "Loading routing file...";
        assert(gr_file.load(GlobalParams::routing_filename.c_str()));
        std::cout << "Done" << endl;
    }
    // Check for traffic table availability
    if (GlobalParams::traffic_distribution == TRAFFIC_TABLE_BASED) {
        std::cout << "Loading traffic table...";
        assert(gttable.load(GlobalParams::traffic_filename.c_str()));
        std::cout << "Done" << endl;
    }
    // Check for traffic trace availability
    else if (GlobalParams::traffic_distribution == TRAFFIC_TRACE_BASED) {
        std::cout << "Loading neuron node from "
                  << GlobalParams::neuron_node_filename << "...";
        assert(nn_parser.loadNodeMap(GlobalParams::neuron_node_filename.c_str()));
        std::cout << "Done" << endl;
        if (GlobalParams::cluster_traffic == false) {
            std::cout << "Loading neural network from "
                      << GlobalParams::neural_network_filename << "...";
            assert(nn_parser.loadConnection(
                GlobalParams::neural_network_filename.c_str()));
            std::cout << "Done" << endl;
        }
        gtrtable.ptr_nn_parser = &nn_parser;
        std::cout << "Loading traffic trace...";
        assert(gtrtable.load(GlobalParams::traffic_filename.c_str()));
        std::cout << "Done" << endl;
    }
    ...
    // Initialize signals
    int dimX = GlobalParams::dim_x + 1;
    int dimY = GlobalParams::dim_y + 1;
    ...
    t = new Tile **[GlobalParams::dim_x];
    for (int i = 0; i < GlobalParams::dim_x; i++) {
        t[i] = new Tile *[GlobalParams::dim_y];
    }
    ...
    for (int j = 0; j < GlobalParams::dim_y; j++) {
        for (int i = 0; i < GlobalParams::dim_x; i++) {
            ...
            bool tile_rout_only =
                (find(GlobalParams::rout_only_nodes.begin(),
                      GlobalParams::rout_only_nodes.end(),
                      tile_id) != GlobalParams::rout_only_nodes.end());
            t[i][j] = new Tile(tile_name, tile_id, tile_rout_only);

            // Tell to the router its coordinates
            t[i][j]->r->configure(j * GlobalParams::dim_x + i,
                                  GlobalParams::stats_warm_up_time,
                                  GlobalParams::buffer_depth, grtable, gr_file);
            ...
            if (!tile_rout_only) {
                ...
                t[i][j]->pe->local_id = j * GlobalParams::dim_x + i;
                ...
                if (GlobalParams::traffic_distribution == TRAFFIC_TRACE_BASED)
                    t[i][j]->pe->never_transmit =
                        (gtrtable.occurrencesAsSource(t[i][j]->pe->local_id) == 0);
                else
                    t[i][j]->pe->never_transmit =
    ...
    for (int i = 0; i <= GlobalParams::dim_x; i++) {
        req[i][0].south = 0;
        ack[i][0].north = 0;
        req[i][GlobalParams::dim_y].north = 0;
        ack[i][GlobalParams::dim_y].south = 0;

        free_slots[i][0].south.write(NOT_VALID);
        free_slots[i][GlobalParams::dim_y].north.write(NOT_VALID);

        nop_data[i][0].south.write(tmp_NoP);
        nop_data[i][GlobalParams::dim_y].north.write(tmp_NoP);
    }

    for (int j = 0; j <= GlobalParams::dim_y; j++) {
        req[0][j].east = 0;
        ack[0][j].west = 0;
        req[GlobalParams::dim_x][j].west = 0;
        ack[GlobalParams::dim_x][j].east = 0;

        free_slots[0][j].east.write(NOT_VALID);
        free_slots[GlobalParams::dim_x][j].west.write(NOT_VALID);

        nop_data[0][j].east.write(tmp_NoP);
        nop_data[GlobalParams::dim_x][j].west.write(tmp_NoP);
    }

    // invalidate reservation table entries for non-exhistent channels
    for (int i = 0; i < GlobalParams::dim_x; i++) {
        t[i][0]->r->reservation_table.invalidate(DIRECTION_NORTH);
        t[i][GlobalParams::dim_y - 1]->r->reservation_table.invalidate(
            DIRECTION_SOUTH);
    }
    for (int j = 0; j < GlobalParams::dim_y; j++) {
        t[0][j]->r->reservation_table.invalidate(DIRECTION_WEST);
        t[GlobalParams::dim_x - 1]
         [j]->r->reservation_table.invalidate(DIRECTION_EAST);
    }
    for (unsigned int n = 0; n < GlobalParams::rout_only_nodes.size(); n++) {
        Coord coord = id2Coord(GlobalParams::rout_only_nodes[n]);
        t[coord.x][coord.y]->r->reservation_table.invalidate(DIRECTION_LOCAL);
    }
}
