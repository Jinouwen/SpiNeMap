...
struct CommHistory {
    ...
    vector < double >network_delays;
    vector < int > hop_counts;
    unsigned int total_received_flits;
    double last_received_flit_time;
    double last_received_flit_generated_time;
    int disorder_flit_count;
    int interarrival_distortion;
};
...
class Stats {
  ...
  public:
    ...
    // Return the number of disorder flit of the current node as
    // regards to the communication whose source is src_id
    int getDisorderFlitCount(int src_id);

    // Return the number of disorder flit of the current node
    ...
    // Returns the average network delay (cycles) for the current node as
    // regards to the communication whose source is src_id
    double getAverageNetworkDelay(const int src_id);

    // Returns the average network delay (cycles) for the current node
    double getAverageNetworkDelay();

    // Returns the average hop count for the current node as
    // regards to the communication whose source is src_id
    double getAverageHopCount(const int src_id);

    // Returns the average hop count for the current node
    ...
    // Returns the max network delay for the current node as regards the
    // communication whose source node is src_id
    double getMaxNetworkDelay(const int src_id);

    // Returns the max network delay (cycles) for the current node
    double getMaxNetworkDelay();

    // Returns the max hop count for the current node as regards the
    // communication whose source node is src_id
    double getMaxHopCount(const int src_id);

    // Returns the max hop count for the current node
    ...
    static Flit max_delay_flit;
    static Flit getMaxDelayFlit();
