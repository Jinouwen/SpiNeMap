...
inline ostream &operator<<(ostream &os, const Flit &flit) {
    ...
    if (GlobalParams::verbose_mode == VERBOSE_HIGH) {
        ...
        os << flit.timestamp << ", " << flit.sequence_no << ", " << flit.src_id << "->" << flit.dst_id << ")";
    }

    return os;
}

// Output overloading for Spike
inline ostream &operator<<(ostream &os, const Spike &spike)
{
    os<<"("<<spike.src_id<<", "<<spike.spike_id<<", "<<spike.spiking_time<<", "<<spike.intra_seg_hop_num<<", "<<spike.inter_seg_hop_num<<")";
...
// Trace overloading for spikes
inline void sc_trace(sc_trace_file *&tf, const Spike &spike, string &name) {
    sc_trace(tf, spike.src_id, name + ".src_id");
    sc_trace(tf, spike.spike_id, name + ".spike_id");
    sc_trace(tf, spike.spiking_time, name + ".spiking_time");
    sc_trace(tf, spike.intra_seg_hop_num, name + ".intra_seg_hop_num");
    sc_trace(tf, spike.inter_seg_hop_num, name + ".inter_seg_hop_num");
}
...
inline Coord id2Coord(int id) {
    ...
    coord.x = id % GlobalParams::dim_x;
    coord.y = id / GlobalParams::dim_x;

    assert(coord.x < GlobalParams::dim_x);
    assert(coord.y < GlobalParams::dim_y);

    return coord;
}

inline int coord2Id(const Coord &coord) {
    int id = (coord.y * GlobalParams::dim_x) + coord.x;

    assert(id < GlobalParams::dim_x * GlobalParams::dim_y);

    return id;
}

inline int getDistance(int src_id, int dst_id) {
    return abs(id2Coord(src_id).x - id2Coord(dst_id).x) + abs(id2Coord(src_id).y - id2Coord(dst_id).y);
}
