...
def testFormat(argv):
    ...
    if len(argv) > 1:
        if argv[1] == '-h':
            print('extract_spike_times.py <inputfile> <outputFilePath>')
            sys.exit()

    if(len(argv) != 3):
        print('Input format is ---> python<v> extract_spike_times.py <inputfile> <outputfilePath>')
        sys.exit(2)
...
if __name__ == "__main__":
    ...
    a = np.load(inputFilePath+'0.npz', allow_pickle=True)
    ...
    for item in lst:
        if item == 'spiketrains_n_b_l_t':
            neuronId = 0
            file = open(outputFilePath+'spike_info.txt', 'w')
            ...
            for layer in spikeInfo:
                ...
                if len(layerInfo) == 5: 
                        ...
                        for x_dim in range(layerInfo[1]):
                            for y_dim in range(layerInfo[2]):
                                for timex in range(layerInfo[3]):
                                    file.write(str(neuronId)+' '+ str(np.count_nonzero(layer[0][time][x_dim][y_dim][timex]))+'\n')
                    ...
                    for time in range(1):
                        for x_dim in range(layerInfo[1]):
                            file.write(str(neuronId)+' '+ str(np.count_nonzero(layer[0][time][x_dim]))+'\n')
